...
## Commande docker
Il est nécessaire d'avoir le docker-compose.yml à la racine. Dans le terminal à cette racine, il faut build le docker via la commande : 
```bash
docker-compose build
``` 
Et enfin, lancer le docker via la commande : 
```bash
docker-compose up
```
