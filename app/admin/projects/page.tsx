...
export default function AdminProjectsPage() {
  ...
  // État pour les ressources des projets (exactement comme dans admin details)
  const [projectResources, setProjectResources] = useState<{
    [key: string]: any;
  }>({});

  // Fonction pour récupérer les ressources d'un projet (exactement comme dans admin details)
  const fetchProjectResources = async (projectId: string) => {
    try {
      console.log(`🔄 Récupération des ressources pour le projet ${projectId}`);
      const data = await getProjectResources(projectId);
      console.log(`✅ Ressources récupérées pour ${projectId}:`, data);
      setProjectResources((prev) => ({
        ...prev,
        [projectId]: data,
      }));
    } catch (e) {
      console.warn(`❌ Erreur pour le projet ${projectId}:`, e);
    }
  };
  ...
  // Précharger les ressources de tous les projets
  useEffect(() => {
    if (projects.length > 0) {
      console.log("🚀 Préchargement des ressources pour tous les projets...");
  ...
  const handleShowMore = () => {
    setDisplayCount((prev) => Math.min(prev + 8, filteredProjects.length));
  };
  ...
  // Gestion du tri et de l'expansion (exactement comme dans admin details)
  const handleCardToggle = async (projectId: string) => {
    if (expandedCard === projectId) {
      // Fermer la carte
      setExpandedCard(null);
    } else {
      // Si une autre carte est déjà ouverte, la fermer d'abord
      ...
      if (!projectResources[projectId]) {
        console.log(
          `📥 Ouverture de la carte ${projectId} - récupération des ressources`
        );
  ...
  return (
    <div className="min-h-screen px-4 sm:px-8 lg:px-16 py-6 sm:py-8 lg:py-12">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-4 gap-8 mb-12 transition-all duration-300">
        {displayedProjects.map((project: any) => {
          // Debug: afficher les ressources avant de les passer au composant
          console.log(`🎯 Projet ${project.name} (${project.id}):`, {
            projectResources: projectResources[project.id],
            resources: projectResources[project.id]?.resources,
            resourcesLength: projectResources[project.id]?.resources?.length,
          });
              ...
              trophies={[
                {
                  name: "Premier commit",
                  obtained:
                    (projectResources[project.id]?.resources?.length || 0) > 0,
                  description: "Premier commit réalisé",
                },
                {
                  name: "Architecture validée",
                  obtained:
                    (projectResources[project.id]?.resources?.length || 0) > 2,
                  description: "Architecture du projet validée",
                },
                {
                  name: "MVP terminé",
                  obtained:
                    (projectResources[project.id]?.resources?.length || 0) > 5,
                  description: "Version minimale viable terminée",
                },
                {
                  name: "Tests passants",
                  obtained:
                    (projectResources[project.id]?.resources?.length || 0) > 8,
                  description: "Tous les tests passent",
                },
                {
                  name: "Projet livré",
                  obtained:
                    (projectResources[project.id]?.resources?.length || 0) > 10,
      ...
      <div className="mt-6">
        <ShowMoreLessButtons
          showMoreVisible={displayCount < filteredProjects.length}
