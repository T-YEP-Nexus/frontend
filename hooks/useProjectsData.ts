...
export function useProjectsData() {
  ...
  const fetchActiveProjects = async () => {
    try {
      setLoading(true);
      const response = await fetch("http://localhost:3008/projects/active/list");
  ...
  const fetchProjectsByStudent = async (studentId?: string) => {
    try {
      ...
      const response = await fetch(`http://localhost:3008/project-students/student/${userId}`);
      ...
      if (result.success) {
        ...
        const projectDetails = await Promise.all(
          result.data.map(async (assignment: ProjectStudent) => {
            const projectResponse = await fetch(`http://localhost:3008/projects/${assignment.id_project}`);
  ...
  const fetchProjectById = async (projectId: string) => {
    try {
      setLoading(true);
      const response = await fetch(`http://localhost:3008/projects/${projectId}`);
